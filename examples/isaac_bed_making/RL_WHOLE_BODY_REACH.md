...
We close it with **[robotics-connect](https://github.com/arm/robotics-connect)** — Arm's Unitree G1
...
![The settled cover, tabs hidden — both free-base G1s at the bedside over the clean, still cover (the rigid grab tabs are now hidden from the render, so no cubes stud the sheet), the near robot reaching onto the head edge where the short-range hand sensor fires the grasp. The hold is a compliant D6 spring joint to a hidden tab, so the robot feels the cover's load and balances against it — no pin, no teleport.](media/rl/spring_grip_engaged.png)
