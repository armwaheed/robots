"""MHS driver for a Unitree G1 swarm peer in the bed-making demo.

Both G1s mount on the Model Hardware Standard (MHS) fabric as **equal peers**. There is no
...
every peer can *see* every other peer over MHS and can ask any
peer for help or offer help to any peer that asks.

Coordination happens two ways, both visible to any fleet client on the fabric:

1. **Callable actions (``@rpc``).** The peer's high-level behaviours are
   exposed as MHS procedures so they are discoverable and invocable by any
   fleet client or orchestrator on the fabric:
...
2. **Broadcast events (``@emit`` / ``@on``).** When a peer claims, picks up,
   places, or releases a corner — or asks/offers help — it emits an MHS
   event. Every other peer subscribes and updates its local view of
   the swarm so it can react (pick a different corner, walk over to support,
   etc.).

Because the fabric keeps **no persistent event history** (events are a live
stream), each driver keeps its own bounded,
...
whom for help" are queryable from any fleet client at any time.
...
to the simulator; the MHS asyncio thread mutates it when peer events
arrive.
"""
...
# The device surface (MhsDriver base, @rpc/@emit/@on/@periodic, identity/status) comes from the
# local mhs_sidecar shim, which binds it to the Model Hardware Standard SDK (mhs). This keeps the
# example self-contained and off any strands_robots internals; see mhs_sidecar.py for the
# the device-surface import shim and migration notes.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from mhs_sidecar import (  # noqa: E402
    DeviceDriver, emit, on, periodic, rpc, DeviceIdentity, DeviceStatus,
)
from mhs_trace import BROADCAST, EVENT_DELIVER, EVENT_PUBLISH  # noqa: E402

logger = logging.getLogger(__name__)


def traced_on(event_name: str):
    """``@on``, plus a message-flow trace record for every delivery.

    Composed at class-definition time rather than by patching handlers onto the instance: the
    subscription machinery (both the loopback bus and the SDK's ``mount``) discovers handlers with
    ``inspect.ismethod``, so a handler replaced by a plain function on the instance simply stops
    being found and the events go nowhere."""

    def decorate(fn):
        async def wrapper(self, device_id: str, evt: str, payload: dict):
            if self._trace is not None:
                self._trace.record(EVENT_DELIVER, src=device_id, dst=self._device_id,
                                   name=evt or event_name, payload=payload)
            return await fn(self, device_id, evt, payload)

        wrapper.__name__ = fn.__name__
        wrapper.__qualname__ = fn.__qualname__
        wrapper.__doc__ = fn.__doc__
        return on(event_name=event_name)(wrapper)
...
class SwarmAgent:
    """Per-robot decision state shared between the MHS driver and the sim loop.

    The sim main thread reads ``state`` to decide simulator actions. The MHS
    driver mutates ``state`` from the asyncio thread when peer events arrive,
    and the read/write RPCs mutate it too. All mutations are guarded by
    ...
    def log_help(self, *, kind: str, frm: str, to: str = "", corner: str = "", reason: str = "") -> dict:
        ...
        history so a fleet client can show either view. ``kind`` is one of
    ...
    # ── Mutators called from the MHS driver's @on handlers (asyncio thread) ──
...
class BedMakingG1Driver(DeviceDriver):
    """MHS driver for one Unitree G1 swarm peer.
    ...
    def __init__(
        ...
        *,
        ...
        # the swarm coordinates offline as it would over NATS. (One gap: the
        # bus does not schedule @periodic methods — the heartbeat only runs
        # when the SDK mounts the driver on a real fabric.)
    ...
    def set_trace(self, trace: Optional[Any]) -> None:
        """Register an ``mhs_trace.MessageTrace`` to record this peer's outgoing events.

        Needed only on the broker path: there the ``@emit`` decorator hands the event straight to
        the MHS runtime, so there is no sink to observe it. On the loopback path the bus records
        publishes itself."""
        self._trace = trace
    ...
    def status(self) -> DeviceStatus:
        ...
        # Fleet dashboards typically treat availability == "available" as the
        # ready state and anything else as busy/offline, so advertise
        # "available" when idle and "busy" while actively working.
        return DeviceStatus(
    ...
    async def _safe_emit(self, name: str, **payload: Any) -> None:
        """Emit an event, tolerating a missing/unconnected transport.
        ...
        if method is not None:
            try:
                ...
                if self._trace is not None:
                    self._trace.record(EVENT_PUBLISH, src=self._device_id, dst=BROADCAST,
    ...
    async def getStatus(self) -> dict:
        """Return the agent's full swarm snapshot."""
    ...
    @rpc(estop=True, labels={"direction": "write", "safety": "critical"})
